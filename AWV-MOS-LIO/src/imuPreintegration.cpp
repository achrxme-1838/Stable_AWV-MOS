...
public:
    ...
    IMUPreintegration()
    {
        ...
        // std::cout << "imu2Lidar Rotation Matrix: \n" << imu2Lidar.rotation().matrix() << std::endl;
        // std::cout << "imu2Lidar Translation: x: " << imu2Lidar.translation().x() << ", y: " << imu2Lidar.translation().y() << ", z: " << imu2Lidar.translation().z() << std::endl;

        // std::cout << "lidar2Imu Rotation Matrix: \n" << lidar2Imu.rotation().matrix() << std::endl;
        // std::cout << "lidar2Imu Translation: x: " << lidar2Imu.translation().x() << ", y: " << lidar2Imu.translation().y() << ", z: " << lidar2Imu.translation().z() << std::endl;
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        double currentCorrectionTime = odomMsg->header.stamp.toSec();
        ...
        if (systemInitialized == false)
        {
            ...
            {
                if (imuQueOpt.front().header.stamp.toSec() < currentCorrectionTime - delta_t)
                {
        ...
        // pose_covariance = optimizer.marginalCovariance(X(key));
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // gtsam::Matrix pose_covariance = imuIntegratorImu_->preintMeasCov();
        // for(int row = 0; row < 6; row++)
        // {
        //     for(int col = 0; col < 6; col++)
        //     {
        //         int new_row = (row < 3) ? row + 3 : row - 3;
        //         int new_col = (col < 3) ? col + 3 : col - 3;

        //         odometry.pose.covariance[new_row * 6 + new_col] = sqrt(pose_covariance(row, col));
        //     }
        // }
